...
describe('сводка', () => {
  ...
  it('суммы по валютам только для активных', () => {
    const items = [
      sub({ id: '1', price: 300, currency: 'USD' }),
      sub({ id: '2', price: 1200, billingPeriod: 'yearly' }),
      sub({ id: '3', price: 499 }),
      sub({ id: '4', price: 999, status: 'paused' }),
    ]
    expect(getMonthlyTotals(items)).toEqual([
      { currency: 'RUB', amount: 599 },
